...
export class Sink {
   ...
   * @memberOf Sink
   */
  protected deleteData(key: string) { return; }

  protected hasChanged(key: string, geoJson: GeoJSON.FeatureCollection<GeoJSON.GeometryObject>) {
    if (!geoJson) return false;
    let hash = Sink.hash(geoJson);
